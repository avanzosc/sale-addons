...
class ProjectProject(models.Model):
    ...
    def _compute_total_out_invoice(self):
        for project in self:
            ...
            amount_collected = 0
            if "domain" in action_window and action_window.get("domain", False):
                invoices = self.env["account.move"].search(action_window.get("domain"))
                if invoices:
                    out_invoices = invoices.filtered(
                        lambda x: x.move_type == "out_invoice" and x.state != "cancel"
                    )
                    if out_invoices:
                        amount_total = sum(out_invoices.mapped("amount_total"))
                        amount_residual = sum(out_invoices.mapped("amount_residual"))
                        out_invoice_amount_total += amount_total
                        amount_collected += amount_total - amount_residual
                    out_refund_invoices = invoices.filtered(
                        lambda x: x.move_type == "out_refund" and x.state != "cancel"
                    )
                    if out_refund_invoices:
                        amount_total = sum(out_refund_invoices.mapped("amount_total"))
                        amount_residual = sum(
            ...
            if "domain" in action_window and action_window.get("domain", False):
                ...
                if invoices:
                    in_invoices = invoices.filtered(
                        lambda x: x.move_type == "in_invoice" and x.state != "cancel"
                    )
                    if in_invoices:
                        amount_total = sum(in_invoices.mapped("amount_total"))
                        amount_residual = sum(in_invoices.mapped("amount_residual"))
                        in_invoice_amount_total += amount_total
                        amount_paid += amount_total - amount_residual
                    in_refund_invoices = invoices.filtered(
                        lambda x: x.move_type == "in_refund" and x.state != "cancel"
                    )
                    if in_refund_invoices:
                        amount_total = sum(in_refund_invoices.mapped("amount_total"))
                        amount_residual = sum(
