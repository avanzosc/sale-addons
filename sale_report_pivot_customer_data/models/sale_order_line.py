...
class SaleOrderLine(models.Model):
    ...
    partner_state_id = fields.Many2one(
        ...
        related='order_partner_id.state_id', store=True)
    partner_city = fields.Char(
        string='City', related='order_partner_id.city', store=True)
    order_partner_shipping_id = fields.Many2one(
        string='Delivery address', related='order_id.partner_shipping_id',
        comodel_name='res.partner', store=True)
    partner_shipping_state_id = fields.Many2one(
        comodel_name='res.country.state', string='Delivery address state',
        related='order_partner_shipping_id.state_id', store=True)
