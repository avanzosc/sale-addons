...
class SaleOrder(models.Model):
    ...
    def _default_type_id(self):
        if self.env.context.get("default_is_offer_type"):
    ...
    def onchange_type_id(self):
        for order in self:
            order.is_offer_type = (
                order.type_id.is_offer_type if order.type_id else False
            )
    ...
    def action_offer_to_quotation(self):
        for offer in self.filtered(lambda x: x.is_offer_type):
            ...
            if not normal_type:
                raise UserError(_("No normal sale order type found."))
            default = {
    ...
    def onchange_stage(self):
        ...
        for order in self:
            if order.stage == "accepted":
                order.acceptance_date = today
                order.rejection_date = False
            elif order.stage == "rejected":
                order.rejection_date = today
                order.acceptance_date = False
            else:
                order.acceptance_date = False
                order.rejection_date = False
