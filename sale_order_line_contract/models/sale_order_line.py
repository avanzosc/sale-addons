...
class SaleOrderLine(models.Model):
    ...
    def create(self, vals):
        ...
        if (
            "no_update_contract_line" not in self.env.context
            and line.contract_line_id
            and (
                not line.contract_line_id.sale_order_line_id
                or line.contract_line_id.sale_order_line_id.id != line.id
            )
        ):
    ...
    def write(self, vals):
        ...
        if (
            "no_update_contract_line" not in self.env.context
            and "contract_line_id" in vals
            and vals.get("contract_line_id", False)
        ):
            for line in self:
