...
class SaleOrder(models.Model):
    ...
    contract_created = fields.Integer(string="Contract created", default=0)
    contract_modified = fields.Integer(string="Contract modified", default=0)
    contract_created_literal = fields.Text(string="Contract created literal")
        ...
        for sale in self:
            ...
            if lines:
                ...
                if sale.contract_modified:
                    ...
                    lit = (
                        lit_message2 if not lit else "{}\n{}".format(lit, lit_message2)
                    )
                if lit:
    ...
    def _catch_values_for_create_contract(self, line):
        name = _("{}: {}").format(self.name, self.partner_id.name)
        vals = {"name": name, "partner_id": self.partner_id.id, "line_recurrence": True}
        if line.product_id.apply_recurrence_in == "contract":
            vals.update(
                {
