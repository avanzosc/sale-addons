...
class StockQuant(models.Model):
    ...
    def _gather(self, product_id, location_id, lot_id=None, package_id=None,
        ...
        if quants and move.ram_id:
            quants = quants.filtered(
                lambda x: x.ram_id and x.ram_id == move.ram_id)
        if quants and move.storage1_size_id:
            quants = quants.filtered(
                lambda x: x.storage1_size_id and
                x.storage1_size_id == move.storage1_size_id)
        if quants and move.grade_id:
            quants = quants.filtered(
                lambda x: x.grade_id and x.grade_tested and
                x.grade_id == move.grade_id and x.grade_tested == "ok")
        return quants
