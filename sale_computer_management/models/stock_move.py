...
class StockMove(models.Model):
    ...
    def _update_reserved_quantity(self, need, available_quantity, location_id,
        ...
        return super(
            StockMove, self.with_context(
                origin_move=self))._update_reserved_quantity(
    ...
    def _get_available_quantity(self, location_id, lot_id=None,
        ...
        return super(
            StockMove, self.with_context(
                origin_move=self))._get_available_quantity(
    ...
    def _compute_allowed_lots(self):
        for move in self:
            ...
            quants = self.env["stock.quant"].with_context(
                origin_move=move)._gather(move.product_id, move.location_id)
            if quants:
