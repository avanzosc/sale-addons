...
class StockPicking(models.Model):
    ...
    no_upadate_returns = fields.Boolean(
        string="No update return", default=False)

    def button_validate(self):
        for picking in self:
            lines = picking.move_line_ids_without_package.filtered(lambda c: c.qty_done != 0)
            if not lines:
                ...
                for line in picking.move_ids_without_package.filtered(
                    ...
                    if line and (
                        line.purchase_line_id) and (
                            line.purchase_line_id.sale_order_line_ids):
                        qty = line.quantity_done
                        i = 0
                        sales = (
                            line.purchase_line_id.sale_order_line_ids.sorted(
                                "create_date"))
                        sale_line = []
                ...
                for line in picking.move_ids_without_package.filtered(
                    ...
                    while qty > 0:
                        pending_negative = self.env["sale.order.line"].search([
                            ...
                            ("pending_qty", "<", 0),
                            ("price_unit", "=", 0)], limit=1)
                        if pending_negative and abs(
                            pending_negative.pending_qty) >= (
                                qty):
                            pending_negative.pending_qty += qty
                            qty = 0
                        elif pending_negative and abs(
                            pending_negative.pending_qty) < (
                                qty):
                            ...
                            qty = 0
            elif picking.picking_type_code == (
                "incoming") and (
                    picking.no_upadate_returns):
