...
class StockReturnPicking(models.TransientModel):
        ...
        if self.env.context.get('active_id') and self.env.context.get('active_model') == 'sale.order':
            ...
            if sale.exists():
                ...
                if len(sale.picking_ids) != 1:
                    raise ValidationError(_("You may only return one picking at a time."))
    ...
    def _create_returns(self):
        ...
        for line in self.picking_id.move_line_ids_without_package.filtered(lambda c: c.qty_done == 0):
            if line.lot_id:
                return_movelines = (
