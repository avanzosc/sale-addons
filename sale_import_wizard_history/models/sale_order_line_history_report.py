...
class SaleOrderLineHistoryReport(models.Model):
    ...
    sales_team = fields.Char(readonly=True)
    ...
    subtotal = fields.Float(readonly=True, aggregator="sum")
    total = fields.Float(readonly=True, aggregator="sum")
    subtotal_min = fields.Float(readonly=True, aggregator="min")
    subtotal_max = fields.Float(readonly=True, aggregator="max")
    total_min = fields.Float(readonly=True, aggregator="min")
    total_max = fields.Float(readonly=True, aggregator="max")
    ...
    def _select_sale_lines(self):
        return """
            SELECT
                ...
                COALESCE(rcp.country_id, rp.country_id) AS customer_country_id,
                ...
                sol.price_total AS item_total_amount,
                sol.price_subtotal AS subtotal,
                sol.price_total AS total,
                sol.price_subtotal AS subtotal_min,
                sol.price_subtotal AS subtotal_max,
                sol.price_total AS total_min,
                sol.price_total AS total_max
            FROM sale_order_line sol
            JOIN sale_order so ON so.id = sol.order_id
            LEFT JOIN sale_order_import soi ON soi.id = so.sale_import_id
            LEFT JOIN res_partner rp ON rp.id = so.partner_id
            LEFT JOIN res_partner rcp ON rcp.id = rp.commercial_partner_id
    ...
    def _select_historical_lines(self):
        return """
            SELECT
                ...
                rp.commercial_partner_id AS commercial_partner_id,
                sil.sales_team_import::text AS sales_team,
