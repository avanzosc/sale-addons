...
class TestSaleOrderLineAttachment(common.SavepointCase):
    ...
    def test_sale_order_line_attachment(self):
        ...
        my_domain = (
            "[('res_model', '=', 'product.template'), "
            f"('res_id', 'in', [{self.product.product_tmpl_id.id}])]"
        )
        ...
        my_domain = (
            ...
            f"('res_id', 'in', [{sale.order_line[0].id}])]"
        )
        self.assertEqual(domain, my_domain)
        wiz = self.wiz_obj.create({})
        result = wiz.with_context(
            active_model="sale.order", active_id=sale.id
        ).onchange_template_id(self.mail_template.id, "comment", "sale.order", sale.id)
