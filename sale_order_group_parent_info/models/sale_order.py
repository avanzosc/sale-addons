...
class SaleOrder(models.Model):
    ...
    main_parent_id = fields.Many2one(
        ...
        related="partner_id.commercial_partner_id", store=True, copy=False
    )
    main_state_id = fields.Many2one(
        string="Main Province", comodel_name="res.country.state",
        compute="_compute_main_state_id", store=True, copy=False
    )
    ...
    def _compute_main_state_id(self):
        for sale in self:
            sale.main_state_id = (
                sale.main_parent_id.state_id.id if sale.main_parent_id.state_id
                else False
            )
