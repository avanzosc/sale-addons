    ...
    def button_return_picking(self):
        if not self.picking_ids and any(
            ...
            if not self.type_id:
                ...
                raise ValidationError(_("You must enter the picking type of the order type."))
            elif not self.type_id.picking_type_id.return_picking_type_id:
                raise ValidationError(_("You must enter the return picking type of picking type of the order type."))
            else:
                self.action_confirm()
                ...
                picking = self.env["stock.picking"].create({
                    ...
                    "picking_type_id": self.type_id.picking_type_id.return_picking_type_id.id,
                    "location_id": self.type_id.picking_type_id.return_picking_type_id.default_location_src_id.id or self.env.ref('stock.stock_location_suppliers').id,
                    "location_dest_id": self.type_id.picking_type_id.return_picking_type_id.default_location_dest_id.id or self.env.ref('stock.stock_location_customers').id,
                    "company_id": self.company_id.id
                })
                picking.onchange_picking_type()
                picking.group_id.sale_id = self.id
                for line in self.order_line:
                    previous_product_uom_qty = {l.id: l.product_uom_qty for l in line}
