...
class StockPicking(models.Model):
    ...
    count_analytic_lines = fields.Integer(
        string="Count analytic lines",
        compute="_compute_count_analytic_lines")

    def _compute_count_analytic_lines(self):
        for picking in self:
            picking.count_analytic_lines = len(
                picking._get_count_analytic_lines())
    ...
    def _get_count_analytic_lines(self):
        ...
        if self.analytic_account_id:
            cond = [('account_id', '=', self.analytic_account_id.id),
    ...
    def create_analytic_line_from_out_picking(self):
        pickings = self.filtered(
            ...
            x.state == "done" and x.analytic_account_id)
        for picking in pickings:
            for move in picking.move_ids_without_package:
                picking = move.picking_id
                if move.quantity_done > 0:
                    cond = [('stock_move_id', '=', move.id),
                            ('account_id', '=', picking.analytic_account_id.id),
                            ('partner_id', '=', picking.partner_id.id),
                            ('product_id', '=', move.product_id.id),
                            ('unit_amount', '<', 0)]
                    old_move = self.env['account.analytic.line'].search(
                        cond, limit=1)
                    if not old_move:
