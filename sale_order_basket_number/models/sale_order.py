...
class SaleOrder(models.Model):
    ...
    basket_count = fields.Integer(
        "Basket Lines",
        compute="_compute_basket_counts",
        store=True,
    )
    ...
    def _compute_basket_counts(self):
        for order in self:
            order.basket_count = len(
                order.order_line.filtered(lambda line: line.basket)
            )
            order.other_count = len(
                order.order_line.filtered(lambda line: not line.basket)
            )
