...
class SaleOrderLine(models.Model):
    ...
    sale_line_cost = fields.Float(
        string="Line Cost", digits="Product Price", copy=False, store=True,
        compute="_compute_sale_line_cost_margin"
    )
    sale_line_margin = fields.Float(
        string="Line margin ", digits="Product Price", copy=False,
        store=True, compute="_compute_sale_line_cost_margin")

    @api.depends("move_ids", "move_ids.move_line_ids",
                 "move_ids.move_line_ids.cost")
