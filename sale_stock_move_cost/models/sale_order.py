...
class SaleOrder(models.Model):
    ...
    sale_order_cost = fields.Float(
        string="Sale Order Cost", digits="Product Price", copy=False, store=True,
        compute="_compute_sale_order_cost_margin"
    )
    sale_order_margin = fields.Float(
        string="Sale Order margin", digits="Product Price", copy=False,
        store=True, compute="_compute_sale_order_cost_margin")
