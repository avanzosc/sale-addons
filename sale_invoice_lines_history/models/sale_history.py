...
class SaleOrderLineHistory(models.Model):
    ...
    partner_id = fields.Many2one(
        relation="sale.order.history",
        string="Partner",
        readonly=True,
        related="order_id.partner_id",
    )
    partner = fields.Char(string="Partner", readonly=True, related="order_id.partner")
    ...
    product_uom_qty = fields.Float(
        string="Ordered Quantity", default=1.0, readonly=True
    )
    product_uos_qty = fields.Float(
        string="Ordered Quantity (UoS)", default=1.0, readonly=True
    )
