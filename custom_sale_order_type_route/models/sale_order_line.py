...
class SaleOrderLine(models.Model):
    ...
    def _compute_special_partner_id(self):
        for line in self:
            partner = False
            if line.order_id and line.order_id.special_burden and (
        ...
        for line in self:
            if not line.order_id and not line.order_id.type_id:
                raise ValidationError(
                    _("The sale order has not type.")
                    )
            if not line.order_id and line.order_id.type_id and not (
                line.order_id.type_id.burden_picking_type
            ):
                raise ValidationError(
            ...
            if line.order_id.type_id.burden_picking_type not in burden_type:
                burden_type.append(line.order_id.type_id.burden_picking_type)
                picking = self.env["stock.picking"].create({
                    "picking_type_id": (
                        line.order_id.type_id.burden_picking_type.id
                    ),
                    "location_id": line.order_id.type_id.burden_picking_type.default_location_src_id.id,
                    "location_dest_id": line.order_id.type_id.burden_picking_type.default_location_dest_id.id,
                ...
                for same_type_line in same_type_lines:
                    if same_type_line.order_id.special_burden:
                        self.env["stock.move.line"].create({
                            ...
                            "product_uom_id": (
                                same_type_line.product_id.uom_id.id
                            ),
